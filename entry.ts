...
export default async function handler(req) {
  ...
  try {
    ...
    const rowsHtml = infoRows.map(r => `
      <tr>
        <td style="padding:10px 16px;border-bottom:1px solid #1e2a45;color:#8899bb;font-size:12px;white-space:nowrap;">${r.icon} ${r.label}</td>
        <td style="padding:10px 16px;border-bottom:1px solid #1e2a45;color:#e0e8ff;font-size:13px;">${r.value}</td>
      </tr>`).join('');
...
<body style="margin:0;padding:0;background:#060b18;font-family:'Segoe UI',Arial,sans-serif;">
  <div style="max-width:600px;margin:0 auto;padding:32px 16px;">
    ...
    <div style="background:#0d1630;border:1px solid #1e2a45;border-radius:16px;padding:24px;margin-bottom:16px;">
      ...
      <p style="color:#7a90b0;font-size:14px;line-height:1.7;margin:0;">Thank you for choosing <strong style="color:#FF4F00;">Kostas Déménagement</strong>. We have received your move request and our team will contact you shortly.</p>
    </div>
    <div style="background:#0d1630;border:1px solid #1e2a45;border-radius:16px;padding:24px;margin-bottom:24px;">
      ...
      <p style="color:#7a90b0;font-size:14px;line-height:1.7;margin:0;">Merci de choisir <strong style="color:#FF4F00;">Kostas Déménagement</strong>. Nous avons bien reçu votre demande et notre équipe vous contactera sous peu.</p>
    </div>
    ${infoRows.length > 0 ? `
